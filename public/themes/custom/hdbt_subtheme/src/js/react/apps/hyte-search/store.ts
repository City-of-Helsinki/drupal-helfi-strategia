...
import { AddressWithCoordinates } from '@/react/common/AddressSearch';
...
const initialParams: SearchState = {
  ...
  [Components.THEME]: urlParams.getAll(Components.THEME).map(value => ({label: Themes.get(value), value})),
};
...
const selectionsToURLParams = (currentParams: SearchState): URLSearchParams => {
  ...
  Object.entries(currentParams).filter(([key]) => key !== 'addressWithCoordinates').forEach(([key, value]) => {
...
export const initializeAppAtom = atom(null, async (get, set, aggs: aggsType) => {
  set(aggsAtom, aggs);
